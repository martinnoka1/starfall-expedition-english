...
$('missions').onclick=()=>{modal('EXPEDITION MAP','Five signals. One home.','Завърши mисия, за да отключиш следващата. Напредъкът се пази на това устройство.','',()=>{});$('dialogaction').hidden=true;levels.forEach((l,i)=>{let b=document.createElement('button');b.disabled=i>unlocked;b.innerHTML=`<span class="missionnumber">0${i+1}</span><span class="missioncopy">${l.name}<small>${i>unlocked?'Complete the previous sector':best[i]?`Best ${Math.floor(best[i])} sec · Relics ${records[i]?.relics||0}/3`:'Signal awaiting restoration'}</small></span><span>${i>unlocked?'🔒':'★'.repeat(records[i]?.stars||0)||'→'}</span>`;b.onclick=()=>start(i);$('levelbuttons').append(b);});};
...
$('settings').onclick=()=>{if(phase==='playing'){pause();return;}if(phase==='paused'&&action===resume){$('preferences').hidden=false;return;}modal('SETTINGS','Your flight.','Звукът и качеството mогат да се променят и по време на пауза.','BACK TO START',menu);$('preferences').hidden=false;};
...
function resize(){renderer.setSize(innerWidth,innerHeight);camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();if(phase==='playing')controlsVisible(true);}window.addEventListener('resize',resize);resize();build(0);setQuality();$('start').firstChild.textContent=unlocked?'RESUME ЕКСПЕДИЦИЯТА ':'START EXPEDITION ';$('loading').hidden=true;animate();
